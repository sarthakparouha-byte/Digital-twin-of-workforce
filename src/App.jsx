...
function App() {
  ...
  // Fetch scenarios from MongoDB on load
  useEffect(() => {
    const fetchScenarios = async () => {
      try {
        ...
        if (res.ok) {
          ...
          // Map MongoDB _id to id for local usage
          setScenarios(data.map(d => ({ ...d, id: d._id })));
        }
      } catch (err) {
        console.error("Failed to fetch scenarios from backend:", err);
      }
    };
  ...
  const personaMetrics = useMemo(() => computePersonaMetrics(effectiveState), [effectiveState]);
  ...
  const handleRestoreScenario = (savedEffectiveState, savedTools) => {
    setSimState(savedEffectiveState);
    setPurchasedTools([]);
    setActiveView('dashboard');
  };
  ...
  return (
    <div style={{ display: 'flex', height: '100vh', backgroundColor: 'var(--bg)', color: 'var(--text)' }}>
      <Sidebar activeView={activeView} onViewChange={setActiveView} exScore={currentMetrics.exScore} roi={currentMetrics.roi} />
